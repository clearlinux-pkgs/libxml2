...
Version  : 2.11.7
Release  : 120
URL      : https://gitlab.gnome.org/GNOME/libxml2/-/archive/v2.11.7/libxml2-v2.11.7.tar.gz
Source0  : https://gitlab.gnome.org/GNOME/libxml2/-/archive/v2.11.7/libxml2-v2.11.7.tar.gz
...
%prep
%setup -q -n libxml2-v2.11.7
cd %{_builddir}/libxml2-v2.11.7
%patch -P 1 -p1
pushd ..
cp -a libxml2-v2.11.7 build32
popd
pushd ..
cp -a libxml2-v2.11.7 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1711738744
...
CFLAGS="${CFLAGS_GENERATE}" CXXFLAGS="${CXXFLAGS_GENERATE}" FFLAGS="${FFLAGS_GENERATE}" FCFLAGS="${FCFLAGS_GENERATE}" LDFLAGS="${LDFLAGS_GENERATE}" %autogen --disable-static --without-lzma
make  %{?_smp_mflags}
...
make clean
export GOAMD64=v2
CFLAGS="${CFLAGS_USE}" CXXFLAGS="${CXXFLAGS_USE}" FFLAGS="${FFLAGS_USE}" FCFLAGS="${FCFLAGS_USE}" LDFLAGS="${LDFLAGS_USE}" %autogen --disable-static --without-lzma
make  %{?_smp_mflags}
...
%autogen --disable-static --without-lzma --without-python --libdir=/usr/lib32 --build=i686-generic-linux-gnu --host=i686-generic-linux-gnu --target=i686-clr-linux-gnu
make  %{?_smp_mflags}
popd
pushd ../buildavx2/
GOAMD64=v3
...
%autogen --disable-static --without-lzma
make  %{?_smp_mflags}
popd
%check
export LANG=C.UTF-8
...
%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libxml2.so.2.11.7
/usr/lib64/libxml2.so.2
/usr/lib64/libxml2.so.2.11.7

%files lib32
%defattr(-,root,root,-)
/usr/lib32/libxml2.so.2
/usr/lib32/libxml2.so.2.11.7
